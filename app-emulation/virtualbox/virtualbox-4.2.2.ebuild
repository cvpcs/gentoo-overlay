...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="+additions alsa doc extensions headless java pam pulseaudio +opengl python +qt4 rdesktop-vrdp +sdk vboxwebsrv vnc"
...
src_prepare() {
	...
	if use rdesktop-vrdp ; then
		epatch "${FILESDIR}"/${PN}-4.1.18-rdesktop-vrdp.patch

		echo "VBOX_WITH_VRDP := 1" >> LocalConfig.kmk
		echo "VBOX_WITH_VRDP_RDESKTOP := 1" >> LocalConfig.kmk
	fi
}
...
src_install() {
	...
	newinitd "${FILESDIR}"/vboxsvc-initd virtualbox
	newconfd "${FILESDIR}"/vboxsvc-confd virtualbox.example
	...
	if ! use headless ; then
		...
		if use rdesktop-vrdp; then
			...
			fowners root:vboxusers /usr/$(get_libdir)/${PN}/rdesktop-vrdp
			fperms 0750 /usr/$(get_libdir)/${PN}/rdesktop-vrdp
			dosym /usr/$(get_libdir)/${PN}/rdesktop-vrdp /usr/bin/rdesktop-vrdp
		fi
