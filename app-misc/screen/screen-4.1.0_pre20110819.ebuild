...
src_prepare() {
	...
	# (Nov 29 2003 -solar)
	epatch "${FILESDIR}"/"${P}"-int-overflow-fix.patch

	# Bug 31070: configure problem which affects alpha
	# (13 Jan 2004 agriffis)
	epatch "${FILESDIR}"/"${P}"-vsprintf.patch

	# Don't use utempter even if it is found on the system
	epatch "${FILESDIR}"/"${P}"-no-utempter.patch
	...
	epatch "${FILESDIR}"/"${P}"-extend-d_termname-ng2.patch
